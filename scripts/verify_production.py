...
def main() -> int:
    ...
    require('@router.post("/sync-all/start")' in product_api, "parallel source sync start endpoint missing")
    require('@router.get("/sync-all/{job_id}")' in product_api, "parallel source sync status endpoint missing")
